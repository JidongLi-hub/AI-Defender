...
function report() {
  ...
  ALDPValue = parseFloat(document.getElementById("ALD").innerText);
...
//     html2canvas(table, { dpi: 50 }).then(tableCanvas => {
//       // 计算表格转换为图片后的尺寸
//       const tableWidth = pdf.internal.pageSize.getWidth() - 80; // 表格宽度，留出边距
//       const tableHeight = tableCanvas.height * tableWidth / tableCanvas.width; // 根据宽度等比例缩放高度
//
//       // 将表格 canvas 转换为图片，并添加到 PDF 中
//       const tableImgData = tableCanvas.toDataURL('image/png');
//       pdf.addImage(tableImgData, 'PNG', 40, 80, tableWidth, tableHeight);
//
//       // 添加模型质量图片到 PDF 中
//       const qualityImgData = qualityCanvas.toDataURL('image/png');
//       pdf.addImage(qualityImgData, 'PNG', 40, tableHeight + 120, qualityCanvas.width / dpi, qualityCanvas.height / dpi);
//                 // 保存 PDF 文件
//       pdf.save("report_with_table_and_title_image.pdf");
//
//     });
